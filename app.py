...
def clean_text(text):
    # Normalize to lowercase and remove punctuation for better matching
...
def get_bot_reply(user_input):
    ...
    for category, keywords in KEYWORDS.items():
        if category == "subjects":
            # For subjects, check if any subject keyword is inside message
...
with st.form("chat_form", clear_on_submit=True):
    ...
    if submit and user_input.strip():
        ...
        # Immediately rerun to show "typing..." message
...
for user_msg, bot_msg in pairs:
    ...
    if bot_msg["content"] is None:
        ...
        if st.session_state.typing:
            ...
            # After displaying typing once, update message content with reply and reset typing flag
            # We update session state here safely because Streamlit will rerun immediately
            for idx in range(len(st.session_state.messages)-1, -1, -1):
                if st.session_state.messages[idx]["role"] == "bot" and st.session_state.messages[idx]["content"] is None:
                    st.session_state.messages[idx]["content"] = st.session_state.last_bot_reply
                    break
            st.session_state.typing = False
            st.experimental_rerun()
        else:
